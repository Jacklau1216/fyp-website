...
class EnsembleDectector:
  def __init__(self):
    ...
    with open('detector/clf_new.pickle', 'rb') as f:
  ...
  def detect_text(self, text):
    gpt_perplexity, gpt_average_perplexity, gpt_burstiness = self.gpt_analyse_text(text, self.gpt1tokenizer, self.gpt1model)
    gpt2_perplexity, gpt2_average_perplexity, gpt2_burstiness = self.gpt_analyse_text(text, self.gpt2tokenizer, self.gpt2model)
    ...
    roberta_detect = self.robertadetectorpipe(text)
    roberta_predict_result = 0 if roberta_detect[0]["label"] == "LABEL_1" else 1
    lf_predict_result = detect(text,self.lfdetectortokenizer,self.lfdetectormodel,self.device)
    dnagpt_predict_result =  self.dnagptdetector.dna_gpt_detect(text)
    results = [perplexity_predict_result[0],roberta_predict_result, lf_predict_result, dnagpt_predict_result]
    predict_result = []
    for result in results:
      if result is not None:
        predict_result.append(result)
    return ((sum(predict_result)/len(predict_result)) >= 0.5, (sum(predict_result)/len(predict_result)))
